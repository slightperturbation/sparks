...
// 1/map_size
uniform float u_blurRadius = 1.0/512.0; //1.0/512.0; 
float depthMapOffset = 1.0/512.0;
...
void main()
{
    ...
    depth += texture( s_depthMap, f_texCoord + vec2(  depthMapOffset, 0 ) ).r;
    depth += texture( s_depthMap, f_texCoord + vec2( -depthMapOffset, 0 ) ).r;
    depth += texture( s_depthMap, f_texCoord + vec2( 0,  depthMapOffset ) ).r;
    depth += texture( s_depthMap, f_texCoord + vec2( 0, -depthMapOffset ) ).r;
    depth *= 0.2;
