...
void Fluid::loadFromFile( const char* filename )
{
    ...
    std::cerr << "Finished loading file with N=" << m_N << " and size=" << m_size << "\n";
}
...
void Fluid::linearSolveSlow( int boundaryCondition, float* x, float* x_prev, float a, float c )
{
    ...
    //std::cerr << "PRE linear Solve a=" << a << ", invc=" << invC << ",  middle voxel = " << x[index(m_N/2, m_N/2, m_N/2)] << " prev=" << x_prev[index(m_N/2, m_N/2, m_N/2)] << "\n";
    ...
    //std::cerr << "POST linear Solve a=" << a << ", invc=" << invC << ",  middle voxel = " << x[index(m_N/2, m_N/2, m_N/2)] << " prev=" << x_prev[index(m_N/2, m_N/2, m_N/2)] << "\n";
}
