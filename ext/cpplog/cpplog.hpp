...
// Only exit in debug mode if CPPLOG_FATAL_EXIT_DEBUG is set.
    ...
    public:
        ...
        ///DEX: added to allow run-time modification of log-level
        void setLogLevel( const loglevel_t& newLevel ) { m_lowestLevelAllowed = newLevel; }

        ///DEX: added to allow testing before expensive logging
        bool isTrace( void ) { return LL_TRACE >= m_lowestLevelAllowed; }
        bool isDebug( void ) { return LL_DEBUG >= m_lowestLevelAllowed; }
        bool isError( void ) { return LL_ERROR >= m_lowestLevelAllowed; }
        bool isInfo( void ) { return LL_INFO >= m_lowestLevelAllowed; }
        bool isWarn( void ) { return LL_WARN >= m_lowestLevelAllowed; }
